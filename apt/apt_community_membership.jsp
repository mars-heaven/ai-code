...
try {
    ...
    if(strSID.contentEquals("get_membership")){
		...
		String strGender = getRequestParam(request, "Gender");
		String strUUID = getRequestParam(request, "UUID");
		String strReservationChannel = getRequestParam(request, "ReservationChannel");
...
printLog("A", "*** time test - " + strPaymentId + " : 1 구매 시작");	
		...
		try { 
			...
			if(strReservationChannel == null || strReservationChannel.contentEquals("")){
				strReservationChannel = "00";
			}
			...
			// 1. 앱에서 선택한 유효기간이 있으면 사용
			if(strRequestValidityDateFrom != null && strRequestValidityDateFrom.length() >= 8) {

				String strSelectedDateFrom = strRequestValidityDateFrom.substring(0, 8);
				strValidityDateFrom = strSelectedDateFrom + "000000";

				// 앱에서 종료일도 내려온 경우 그대로 사용
				if(strRequestValidityDateTo != null && strRequestValidityDateTo.length() >= 8) {
printLog("A", "purchase_membership 이용권 사용기간 - 앱에서 선택한 날짜 사용 strRequestValidityDateTo != null ");
					String strSelectedDateTo = strRequestValidityDateTo.substring(0, 8);
					strValidityDateTo = strSelectedDateTo + "235959";

				} else {
printLog("A", "purchase_membership 이용권 사용기간 - 앱에서 선택한 날짜 사용 else");
					// 종료일이 없으면 기존처럼 당일권 처리
					strValidityDateTo = strSelectedDateFrom + "235959";
				}
			...
			if(strValiditySeatCount.contentEquals("0")){
				...
				try{
					...
					pstmt.setString(9, strReservationChannel);
