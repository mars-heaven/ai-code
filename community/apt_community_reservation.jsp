...
<%!
	public static class PaymentResult {
		public boolean isPaymentConfirmed = false;
		public boolean isPaymentSaved = false;
	...
	// - 회원권 정산일 저장
	// =========================================================
	private void processReservationPayment(Connection conn,
													...
													boolean isPurchase,
                                                    PaymentResult result) throws Exception {
		...
		// =========================================================
		// STEP 4. 회원권 발급 정상 여부 확인
		// =========================================================
		if(!isPurchase) {
			printLog("A","[STEP 4][FAIL] 결제 필요 건인데 회원권 발급 처리 실패" + " - paymentId : " + strPaymentId);
		...
		if(strMembershipUserListId == null
			...
			printLog("A","[STEP 4][FAIL] 회원권 사용자 ID 없음"
						+ " - membershipUserListId : " + strMembershipUserListId
						+ ", paymentId : " + strPaymentId);
		...
		printLog("A", "[STEP 4][SUCCESS] 예약/회원권 DB 작업 완료, 결제 검증 진행 가능"
					+ " - reservationId : " + strReservationId
					+ ", membershipUserListId : " + strMembershipUserListId);

		// =========================================================
		// STEP 5. Bootpay 결제 검증
		// =========================================================
		printLog("A","[STEP 5][START] Bootpay 결제 verify 시작"
					+ " - receiptId : " + strReceiptId
					+ ", expectedPrice : " + strTotalPrice);
...
printLog("A", "*** time test - " + strPaymentId + " : 5 결제컨펌 종료(우회구간)");
		...
		result.isPaymentConfirmed = true;

		// =========================================================
		// Bootpay confirm 결과값 추출
		// =========================================================
		result.receiptUrl = jsonConfirmResult.get("receipt_url") != null ? jsonConfirmResult.get("receipt_url").toString() : "";

		String strPurchasedAt = jsonConfirmResult.get("purchased_at") != null ? jsonConfirmResult.get("purchased_at").toString() : "";

		result.purchasedDate = convertBootpayDateToDBFormat(strPurchasedAt);

		printLog("A", "[STEP 6][SUCCESS] Bootpay 결제 confirm 성공"
					+ " - paymentId : " + strPaymentId
					+ ", receiptId : " + strReceiptId
					+ ", receiptUrlExists : " + !result.receiptUrl.contentEquals("")
					+ ", purchasedDate : " + result.purchasedDate);

		// =========================================================
		// STEP 7. PARTNER_PAYMENT 저장
		// =========================================================
		...
		strInsertPaymentQuery += " (PAYMENT_ID, RESERVATION_ID, PRICE, TOTAL_PRICE, "
								+ "STATE, DATE, COMMENT, IMAGE, PAYMENT_ITEM, "
								+ "COUPON_USE, INSPECTION, MENU_ID, RECEIPT_ID, "
								+ "USER_ID, RECEIPT_URL, PURCHASED_DATE ) ";
		strInsertPaymentQuery += " VALUES(?, ?, ?, ?, '1', "
		...
		try {
			...
			paymentPstmt = conn.prepareStatement(strInsertPaymentQuery);

			paymentPstmt.setString(1, strPaymentId);
			paymentPstmt.setString(2, strMembershipUserListId);
			paymentPstmt.setString(3, strPrice);
			paymentPstmt.setString(4, strPrice);
			paymentPstmt.setString(5, strPaymentItem);
			paymentPstmt.setString(6, strReceiptId);
			paymentPstmt.setString(7, strUserId);
			paymentPstmt.setString(8, result.receiptUrl);
			paymentPstmt.setString(9, result.purchasedDate);
			...
			int nPaymentRet = paymentPstmt.executeUpdate();

			printLog("A", "[STEP 7][RESULT] PARTNER_PAYMENT INSERT 결과"
						+ " - nPaymentRet : " + nPaymentRet);
...
printLog("A", "[STEP 7][FAIL] PARTNER_PAYMENT INSERT 실패" + " - paymentId : " + strPaymentId);
			...
			if(paymentPstmt != null) {
				...
				try {
					...
					printLog("A", "[STEP 7][WARN] paymentPstmt close 실패" + " - exception : " + closeException.toString());
				}
			}
		}
		...
		if(strSettlementCutoffDay != null && !strSettlementCutoffDay.isEmpty()) {
			...
			String strSettlementDate = sdfyyyyMMdd.format(cal.getTime());

			String strMembershipSettlementUpdateQuery = "UPDATE APT_COMMUNITY_MEMBERSHIP_USER_LIST "
														+ "SET SETTLEMENT_DATE = ? "
			...
			try {
				...
				settlementPstmt = conn.prepareStatement(strMembershipSettlementUpdateQuery);
				settlementPstmt.setString(1, strSettlementDate);
				settlementPstmt.setString(2, strMembershipUserListId);

				int nSettlement = settlementPstmt.executeUpdate();

				printLog("A", "[STEP 7][RESULT] 회원권 정산일 UPDATE"
								+ " - settlementDate : " + strSettlementDate
				...
				if(settlementPstmt != null) {
					...
					try {
						...
						printLog("A", "[STEP 7][WARN] settlementPstmt close 실패" + " - exception : " + closeException.toString());
					}
				}
			}
		}
...
try {
	...
	//예약 취소하기
		...
		if(nRet != 0){
			...
			if(!strPushNotiCode.contentEquals("0")){
				...
				strMsg = strUserDong + "동 "
                        ...
                        + strCommunityCenterName + " 예약을 취소하셨습니다.";
		...
		strQuery += "   '-1', ";
		strQuery += "   TIMESTAMPDIFF(DAY, DATE_FORMAT(SYSDATE(), '%Y%m%d'), cr.DATE) ";
		strQuery += " ) ";
		strQuery += " WHEN (TIME IS NULL OR TIME = '') ";
		strQuery += " THEN IF( ";
		strQuery += "   STR_TO_DATE(LEFT(cr.EXPIRATION_DATE, 8), '%Y%m%d') < CURDATE(), ";
		strQuery += "   '-1', ";
		strQuery += "   DATEDIFF( ";
		strQuery += "     STR_TO_DATE(LEFT(cr.EXPIRATION_DATE, 8), '%Y%m%d'), ";
		strQuery += "     CURDATE() ";
		strQuery += "   ) ";
		strQuery += " ) ";
		strQuery += " END";
		...
		for(int nRow = 0; rs.next(); nRow++) {
			...
			String strMembershipUserListId = "";
			String strMembershipId = "";			// pny 임시조회
			String strSubQuery1 = "SELECT MEMBERSHIP_USER_LIST_ID, MEMBERSHIP_ID FROM APT_COMMUNITY_RESERVE WHERE RESERVE_ID = ?";
			...
			try {
				...
				if (rsSub1.next()) {
					strMembershipUserListId = rsSub1.getString("MEMBERSHIP_USER_LIST_ID") != null ? rsSub1.getString("MEMBERSHIP_USER_LIST_ID") : "";
					strMembershipId = rsSub1.getString("MEMBERSHIP_ID") != null ? rsSub1.getString("MEMBERSHIP_ID") : "";
				}
						...
						if(nCol == 5){
							if(strData.length() >= 16) {
								...
								if(strStartDate.contentEquals(strEndDate)) {
									...
									strData = formatDate(strStartDate)+ " ~ " + formatDate(strEndDate);
								}
            ...
            && !isExpiredByDiffDate){
            	// 임시로 광명자이 독서실 구분 처리
				if(strAptCode.contentEquals("100622") && (strMembershipId.contentEquals("1678") || strMembershipId.contentEquals("1681"))) {
					strQRId = "02";
					strQRSecurityCode = "00001";
				} else {
			...
			if(strOperationHours != null && !strOperationHours.contentEquals("")) {
				...
				String[] arrOperationTime = getOperationTimeRange(strOperationHours, null);
				strFacilityStartTime = arrOperationTime[0];
				strFacilityEndTime = arrOperationTime[1];

			} else {
				// OPERATION_HOURS가 없으면 시설 기본 운영시간 사용
				strFacilityStartTime = strStartTime != null ? strStartTime.replace(":", "") : "";
				strFacilityEndTime = strEndTime != null ? strEndTime.replace(":", "") : "";
			}
			...
			boolean bHasReservationTime = strReservationTime != null && !strReservationTime.contentEquals("") && strReservationTime.length() == 8;

			SimpleDateFormat sdfQRTimeL = new SimpleDateFormat("HHmm");
			...
			if(bHasReservationTime) {
				...
				calQRStartL.set(Calendar.HOUR_OF_DAY, Integer.parseInt(strReservationTime.substring(0, 2)));
				calQRStartL.set(Calendar.MINUTE, Integer.parseInt(strReservationTime.substring(2, 4)));
				...
				calQREndL.set(Calendar.HOUR_OF_DAY, Integer.parseInt(strReservationTime.substring(4, 6)));
				calQREndL.set(Calendar.MINUTE, Integer.parseInt(strReservationTime.substring(6, 8)));
				calQREndL.set(Calendar.SECOND, 0);
				calQREndL.set(Calendar.MILLISECOND, 0);
				// 예약 종료 15분 후까지 입장 가능
				calQREndL.add(Calendar.MINUTE, 15);

				strActualStartTime = sdfQRTimeL.format(calQRStartL.getTime());
				strActualEndTime = sdfQRTimeL.format(calQREndL.getTime());
			...
			while(rsHolidaysL.next()) {
				...
				holidaysListL.add(createHolidayFromResultSet(rsHolidaysL));
			}
			...
			for(Holiday holiday : holidaysListL) {
				...
				} else if(isTodayTempHoliday(holiday,formattedDateYYYYMMDDL)) {
					isTempHolidayL = true;
					isHolidayL = true;
				}
			}
			...
			// 운영시간 유효성 확인
			boolean bValidTimeL = strFacilityStartTime != null
			...
			if(bValidTimeL) {
				nStartTimeL = Integer.parseInt(strFacilityStartTime);
				nEndTimeL = Integer.parseInt(strFacilityEndTime);
			}
			...
			if(strCommunityState != null && strCommunityState.contentEquals(UNAVAILABLE)) {
				strState = "4";
			}

			writeColumn(baOutStream, strQRId);

			writeColumn(baOutStream, strQRSecurityCode);

			writeColumn(baOutStream, strSecurity);

			writeColumn(baOutStream, strActualStartTime);

			writeColumn(baOutStream, strActualEndTime);
		...
		if(rs.next()) {
			...
			strPurchaseDate = rs.getString("REG_DATE") != null ? rs.getString("REG_DATE").trim() : "";
			strReserveCancelTime = rs.getString("RESERVE_CANCEL_TIME") != null ? rs.getString("RESERVE_CANCEL_TIME").trim() : "";
		...
		// 예약 등록일을 구매 날짜로 표시
		if(strPurchaseDate != null && strPurchaseDate.length() == 14) {

			String strFormattedPurchaseDate = formatDateTime(strPurchaseDate);

			if(strFormattedPurchaseDate != null && !strFormattedPurchaseDate.contentEquals("")) {
				strInfo += "구매 날짜*" + strFormattedPurchaseDate + "*+";
			}
		}

		// 취소된 예약일 때만 예약 취소 날짜 표시
		if(isCanceled && strReserveCancelTime != null && strReserveCancelTime.length() == 14) {

			String strFormattedCancelDate = formatDateTime(strReserveCancelTime);

			if(strFormattedCancelDate != null && !strFormattedCancelDate.contentEquals("")) {
				strInfo += "취소 날짜*" + strFormattedCancelDate + "*+";
			}
		}
		...
		if(strOperationHours != null && !strOperationHours.contentEquals("")) {
			...
			strFacilityStartTime = strStartTime != null ? strStartTime.replace(":", "").replace(" ", "") : "";
			strFacilityEndTime = strEndTime != null ? strEndTime.replace(":", "").replace(" ", "") : "";
		}
        ...
        boolean bValidReservationTime = strTime != null && strTime.length() == 8;
        ...
        boolean bValidFacilityTime = strFacilityStartTime != null
                                    && strFacilityEndTime != null
                                    && strFacilityStartTime.length() == 4
                                    && strFacilityEndTime.length() == 4;
        ...
        if(bValidFacilityTime) {
            try {
                nFacilityStartTime = Integer.parseInt(strFacilityStartTime);
                nFacilityEndTime = Integer.parseInt(strFacilityEndTime);
        ...
        if(isHoliday) {
            ...
            ||
            (nFacilityStartTime > nFacilityEndTime
                && (nNowTime > nFacilityStartTime || nNowTime < nFacilityEndTime))
        ) {
		...
		writeColumn(baOutStream, strCancellable);

		// 버튼 텍스트 취소가능하면 취소하기 불가능하면 
		// 예약 mm분전에는 취소가 불가능합니다.
		writeColumn(baOutStream, strButtonContext);

		// 취소 가능 날짜 취소가 불가능해지면 빈 스트링
		writeColumn(baOutStream, strCancellableDate);

		// 버튼 보임 여부 1 : 보임, 0 안 보임
		writeColumn(baOutStream, strButtonVisibility);
		...
		writeColumn(baOutStream, strSeatChangeAble);

		writeColumn(baOutStream, strMembershipId);

		writeColumn(baOutStream, strOptionIds);

		writeColumn(baOutStream, strPlace);

		if(strDate.length() == 16){		
			writeColumn(baOutStream, strDate.substring(0,8));
			writeColumn(baOutStream, strDate.substring(8,strDate.length()));
		}else{
			writeColumn(baOutStream, strDate); // 자리변경시 문제 생기면 다시 빈문자열로 수정 요망
			writeColumn(baOutStream, strDate);
		}
		...
		writeColumn(baOutStream, strCommunitYType);

		if(strReceiptURL == null || strReceiptURL.trim().contentEquals("")){
			writeText(baOutStream, "");
		}else{
			writeText(baOutStream, strReceiptURL);
		}
		...
		if(strMembershipUserListId == null || strMembershipUserListId.trim().contentEquals("")) {
			...
			writeText(baOutStream, strMembershipUserListId);
		}
		writeColumnDel(baOutStream);
	...
	} else if(strSID.contentEquals("get_qr_info_list")) { // QR 정보 조회

		String strAptCode = getRequestParam(request, "AptCode");

		String strQRListQuery = "";
		strQRListQuery += "SELECT q.COMMUNITY_TYPE, ";
		strQRListQuery += "       (SELECT c.TITLE ";
		strQRListQuery += "          FROM APT_COMMUNITY c ";
		strQRListQuery += "         WHERE c.APT_CODE = q.APT_CODE ";
		strQRListQuery += "           AND c.COMMUNITY_TYPE = q.COMMUNITY_TYPE ";
		strQRListQuery += "         LIMIT 1) AS TITLE, ";		
		...
		while(rs.next()){
			...
			writeText(baOutStream, strQRSecurityCode);
			writeRecord(baOutStream);
		}
...
printLog("A", "*** time test - " + strPaymentId + " : 1 예약 시작");
			...
			if(strAptCode.contentEquals("") ||
				...
				strReservationUserName.contentEquals("")) {
					...
					writeColumn(baOutStream, "예약 필수 정보가 없습니다.");
				...
				if(resultCnt.contentEquals("0")){
					if(isNeedPayment) {
						if(strPaymentId.contentEquals("") || strReceiptId.contentEquals("") || strTotalPrice.contentEquals("")) {
							...
							writeColumn(baOutStream, "결제 정보가 없습니다.");
						...
						if(strOrderState.contentEquals("1")) {
							...
							writeColumn(baOutStream, "이미 승인된 결제건입니다.");
					...
					// =========================================================
					// 예약 INSERT
					// - APT_COMMUNITY_RESERVE에 예약 데이터를 생성한다.
			...
			// =========================================================
			// 예약 처리 결과 응답 준비 및 후처리용 기본 데이터 조회
			...
			writeColumnDel(baOutStream);
			writeText(baOutStream, strMessage);

			writeColumnDel(baOutStream);
			writeText(baOutStream, strReservationId);
			
			writeColumnDel(baOutStream);
			writeText(baOutStream, strReservationInfo);
			...
			// =========================================================
			// 예약형/대관형 회원권 발급 처리
			// - RESERVE_TYPE_ADDITIONAL_AFTER_PURCHASE가 아닌 경우 예약 날짜/시간 기준으로 PURCHASE_DATE, EXPIRATION_DATE를 만든다.
			...
			if(isSuccess &&
				...
				// =========================================================
				// 구매 후 추가예약형 회원권 처리
				// - RESERVE_TYPE_ADDITIONAL_AFTER_PURCHASE인 경우 회원권 유효기간 정책을 조회해 현재 시점부터 만료일을 계산한다.
			...
			if(isNeedPayment && isSuccess && nRet == 1) {
				...
				isPaymentConfirmed = paymentResult.isPaymentConfirmed;
				isPaymentSaved = paymentResult.isPaymentSaved;
				strReceiptUrl = paymentResult.receiptUrl;
				strPurchasedDate = paymentResult.purchasedDate;
			}
			...
			// =========================================================
			// 최종 -> 예약 INSERT, 회원권 INSERT, 결제 INSERT, 정산일 UPDATE가 전부 성공했을 때만 DB 반영
			// =========================================================
...
printLog("A", "*** time test - " + strPaymentId + " : 9 얼굴등록 처리 종료");
            ...
            // processReservationPayment 내부에서 예외가 발생했더라도
            // Bootpay confirm 성공 여부는 PaymentResult에 남아있음
            isPaymentConfirmed = paymentResult.isPaymentConfirmed;
            isPaymentSaved = paymentResult.isPaymentSaved;
			...
			if(!isDbCommitted) {
				try {
					...
					writeColumn(baOutStream, strErrorMessage);
		...
		writeColumn(baOutStream, strResult);
		...
		writeColumn(baOutStream, strTitle);
		writeColumn(baOutStream, strReservationNotice);
...
//			}
			...
			writeColumn(baOutStream, jsonString);
			writeColumn(baOutStream, Integer.toString(nRemainingSeats));
...
printLog("A", "*** time test - " + strPaymentId + " : 6 게스트하우스 예약처리 완료");
			...
			writeColumn(baOutStream, strResultInfo);
		...
		ByteArrayOutputStream baOutStream = new ByteArrayOutputStream();
		writeColumn(baOutStream, Integer.toString(nResultRet));
		writeRecord(baOutStream);
		returnData(baOutStream, outStream);
		...
		for(int nRow = 0; rs.next(); nRow++) {
			...
			for(int nCol = 1; nCol <= rsMetaData.getColumnCount(); nCol++) {
				...
				if(nCol <= 8) {
					...
					if(nCol == 7 && strData.startsWith("USE_COUNT:")) {
						...
						writeText(baOutStream, strUseCountText);
						...
						writeText(baOutStream, ("예약 자리 : " + placeValue));

						System.out.println("RAW_DATA = [" + placeValue + "]");

					} else if(nCol == 7 && strData.isEmpty()) {

						writeText(baOutStream, strMembershipName);

					} else {
						writeText(baOutStream, strData);
					}
					writeColumnDel(baOutStream);
				} else if(nCol == 9) {
			...
			if(strReserveTimeRaw != null 
				...
				strTimeForCalc = getOperationTime(strOperationHours, null);
			...
			try {
				...
				while(rsHolidays.next()) {
					holidaysList.add(createHolidayFromResultSet(rsHolidays));
				}
			} finally {
